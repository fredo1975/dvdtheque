pipeline {
    ...
    stages {
        ...
        stage ('Build') {
            steps {
                sh 'mvn -e -X release:clean build-helper:parse-version versions:set release:prepare -DdevelopmentVersion=\\\${parsedVersion.majorVersion}.\\\${parsedVersion.nextMinorVersion}.0-SNAPSHOT release:perform -Darguments="-Djava.io.tmpdir=/var/tmp/exportDir"'
            }
            post {
                success {
