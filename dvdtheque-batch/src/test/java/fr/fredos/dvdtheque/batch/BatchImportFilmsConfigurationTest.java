	...
	public void launchImportFilmsJob() throws Exception {
		...
		for(Film film : films) {
			if(TITRE_FILM_2001.equals(film.getTitre())) {
				...
				FilmBuilder.assertFilmIsNotNull(film,true,0,FilmOrigine.EN_SALLE, "1968/04/09");
			}
			if(TITRE_FILM_2046.equals(film.getTitre())) {
				...
				FilmBuilder.assertFilmIsNotNull(film,true,0,FilmOrigine.DVD, "2004/05/20");
			}
			if(TITRE_FILM_40_ans.equals(film.getTitre())) {
				...
				FilmBuilder.assertFilmIsNotNull(film,false,Long.valueOf(temp).intValue(),FilmOrigine.DVD, "2012/12/20");
			}
		}
